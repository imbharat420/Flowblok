...
import type { BlockNode, ComponentDef } from "@/lib/types";
import {
  Paintbrush,
  Database,
  GitBranch,
...
// The seven block tabs from 04-FRONTEND-SPEC.md.
export const TABS = [
...
const DATA_SOURCES = ["Static", "Database", "API", "Workflow", "AI", "CRM", "Commerce"];
      ...
      <div className="flex-1 overflow-y-auto p-4">
        {!node ? (
          ...
          <>
              ...
              <div className="space-y-3 text-[13px]">
                ...
                <div className="space-y-1.5">
                  {DATA_SOURCES.map((src, i) => (
                    <label key={src} className="flex items-center gap-2 rounded-md border border-border bg-bg px-2.5 py-2">
                      <input type="radio" name="ds" defaultChecked={i === 0} className="accent-[var(--accent)]" />
                      <span className="text-fg">{src}</span>
                    </label>
                  ))}
                </div>
                <div className="rounded-md border border-dashed border-border p-3 text-[12px] text-fg-subtle">
                  Choosing <span className="text-fg">Database</span> lets you pick a table, auto-load its
                  fields, and visually map them (e.g. Card Title ← Product Title). Generates
                  <span className="font-mono"> db.products.findMany()</span> behind the scenes.
                </div>
              </div>
            )}
            ...
            {active === "seo" && (
              <div className="space-y-3 text-[13px]">
                <Field field={{ key: "metaTitle", label: "Meta title", type: "text" }} value="" onChange={() => {}} />
                <Field field={{ key: "metaDesc", label: "Meta description", type: "textarea" }} value="" onChange={() => {}} />
            ...
            {active === "ai" && (
              <div className="space-y-2 text-[13px]">
                ...
                />
                <button className="w-full rounded-md bg-accent py-1.5 text-[13px] font-medium text-accent-fg">
                  Generate
                </button>
              </div>
            )}
      ...
      {field.type === "textarea" ? (
        ...
        <div className="flex items-center gap-2">
          ...
          <input
            value={val}
            onChange={(e) => onChange(e.target.value)}
            className="flex-1 rounded-md border border-border bg-bg px-2.5 py-1.5 font-mono text-[12px] text-fg outline-none"
          />
        </div>
      ) : (
        <input
